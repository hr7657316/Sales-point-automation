...
- `python -m sales_points.quarterly --report "JUNE 2026=<fit report>" ... --m1sx m1sx.csv --pharmacy pharmacy.csv --deductions deductions.csv -o Q3.xlsx` builds the quarterly FIT COMPLETE report in Allissa's RSM layout (MASTER / EAST REGION / WEST REGION x GRAND TOTAL, DME, M1SX, PHARMACY). Region comes from `rules/rep_regions.csv`.
